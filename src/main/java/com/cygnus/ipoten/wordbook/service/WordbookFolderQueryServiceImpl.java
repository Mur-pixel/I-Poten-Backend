...
import com.cygnus.ipoten.wordbook.controller.response_form.FolderSummaryResponseForm;
...
public class WordbookFolderQueryServiceImpl implements WordbookFolderQueryService {
    ...
    public MyFolderListResponseForm getMyFolders(Long accountId) {
        ...
        // 2) DB 조회
        List<FolderCountRow> rows = wordbookFolderRepository.findMyFoldersWithFavoriteCount(accountId);
    ...
    public MyFolderListResponseForm getMyFoldersWithStats(Long accountId) {
        ...
        List<FolderStatsRow> rows = wordbookFolderRepository.findMyFoldersWithStats(accountId);
        var response = MyFolderListResponseForm.builder()
    ...
    public boolean existsByIdAndAccountId(Long folderId, Long accountId) {
        return wordbookFolderRepository.existsByIdAndAccount_Id(folderId, accountId);
    }
     ...
     */
    @Override
    public WordbookFolderService.ExportTermIdsResult collectExportTermIds(Long accountId, Long folderId,
                                                                          String memorization,
        ...
        if (totalBefore == 0) {
            return new WordbookFolderService.ExportTermIdsResult(folderId, List.of(), 0, 0, false, hardLimit, 0);
        }
        ...
        return new WordbookFolderService.ExportTermIdsResult(
                folderId,
                finalIds,
                totalBefore,
                filteredOut,
    ...
    public long countTermsInFolderOrThrow(Long accountId, Long folderId) {
        wordbookFolderRepository.findByIdAndAccount_Id(folderId, accountId)
    ...
    public WordbookTermQueryRepository.PageResult<FolderTermRow> listFolderTerms(
        ...
        return wordbookTermQueryRepository.findFolderTerms(accountId, folderId, page, perPage, sort);
    }
}
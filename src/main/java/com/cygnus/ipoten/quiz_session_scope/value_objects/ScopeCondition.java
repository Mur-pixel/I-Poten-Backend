 ...
 * - JOB           : 직무(JobRole)를 기준으로 퀴즈 세션 생성
 * - SET           : 이미 정의된 퀴즈 세트를 기준으로 퀴즈 세션 생성
 *
 * 구성 요소 의미:
 * - sourceType    : 퀴즈 세션 생성의 기준이 되는 출처 타입
 * - wordbookScope : 단어장 기반 출제 범위 (sourceType=WORDBOOK)
 * - categoryScope : 카테고리 기반 출제 범위 (sourceType=TERM_CATEGORY)
 * - jobScope      : 직무 기반 출제 범위 (sourceType=JOB)
 ...
 * - 퀴즈 세션 생성 로직에서 조건 분기(if/else)를 최소화
 *
 * 설계 의도:
 * - Lombok @Value 기반의 불변(Value Object) 설계
 * - 출제 기준(Source)과 출제 범위(Scope)를 명확히 분리하면서도 하나의 맥락으로 묶음
 * - CategoryScope, JobScope, WordbookScope 등 하위 Scope들의
 *   진입 지점을 단일 객체로 통합하여 도메인 흐름을 단순화
 *
 * 사용 예:
 * - 단어장 기반 퀴즈 세션 생성: ScopeCondition.forWordbook(wordbookScope)
 * - 카테고리 기반 퀴즈 세션 생성: ScopeCondition.forCategory(categoryScope)
 * - 직무 기반 퀴즈 세션 생성: ScopeCondition.forJob(jobScope)
 *
 * 주의 사항:
 * - sourceType에 맞지 않는 Scope 필드는 null로 유지되며,
 *   실제 해석과 검증은 퀴즈 세션 생성 서비스에서 수행된다.
 */
@Value
public class ScopeCondition {

    public enum SourceType {
        WORDBOOK, TERM_CATEGORY, JOB, SET, TOPIC
    }
    ...
    public static ScopeCondition forJob(JobScope jobScope, SeedPolicy seedPolicy) {
        return new ScopeCondition(
                SourceType.JOB,
                null,
                null,
    ...
    public static ScopeCondition forSet(SetScope setScope, SeedPolicy seedPolicy) {
        return new ScopeCondition(SourceType.SET, null, null, null, setScope, seedPolicy);
    }
}

package com.cygnus.ipoten.custom_term_admin.service;
...
public class CustomTermEraseService {
    ...
    /**
     * 삭제 결과 요약 DTO.
     * - folders:        실제 삭제된 wordbook_folder 개수
     ...
     *  3) 마지막으로 wordbook_folder 삭제
     */
    @Transactional
    public Result eraseByAccountId(Long accountId) {
        // 운영 모니터링/점검용 사전 집계. 결과 본문에는 사용하지 않지만 실제 삭제 수와 비교해보기 위해 남겨둠
        long foldersBefore = count("SELECT COUNT(*) FROM wordbook_folder WHERE account_id=:id", accountId);
        long wbtBefore = count("""
            SELECT COUNT(*)
              FROM wordbook_term t
              JOIN wordbook_folder f ON f.id = t.folder_id
        ...
        int delFolders = execute("DELETE FROM wordbook_folder WHERE account_id=:id", accountId);

        log.info(
                "[user-term:erase] accountId={} deleted: folders={}, uwt={}, utp={} (before: folders={}, uwt={}, utp={})",
                accountId, delFolders, delUwt, delUtp,
                foldersBefore, wbtBefore, utpBefore
        );

        // 실제 삭제된 개수만 결과로 반환
        return new Result(delFolders, delUwt, delUtp);
    }

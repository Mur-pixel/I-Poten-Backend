package com.cygnus.iptn.term.repository;
...
public interface TermRepository extends JpaRepository<Term, Long>, JpaSpecificationExecutor<Term> {
    ...
    @Query("""
        ...
        LEFT JOIN com.cygnus.iptn.term.entity.TermTag tt ON tt.term = t
        LEFT JOIN com.cygnus.iptn.term.entity.Tag tag ON tag = tt.tag
    ...
    @Query(
        value = """
          SELECT DISTINCT t
            ...
            JOIN com.cygnus.iptn.term.entity.TermTag tt ON tt.term = t
            JOIN tt.tag tg
    ...
    @Query("""
        ...
        WHERE t.termCategory.id IN :catIds AND (
           ...
           OR LOWER(t.description) LIKE LOWER(CONCAT('%', :q, '%'))
           OR (:includeTags = TRUE AND EXISTS (
                SELECT 1 FROM com.cygnus.iptn.term.entity.TermTag tt
                 WHERE tt.term = t

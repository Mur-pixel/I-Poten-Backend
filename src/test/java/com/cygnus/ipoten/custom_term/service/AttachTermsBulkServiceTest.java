...
import com.cygnus.ipoten.wordbook.service.WordbookFolderServiceImpl;
...
class AttachTermsBulkServiceTest {
    ...
    @InjectMocks
    private WordbookFolderServiceImpl service;

    @Mock private WordbookFolderRepository wordbookFolderRepository;
    @Mock private WordbookTermRepository wordbookTermRepository;
    ...
    private WordbookFolder folderMock;

    @BeforeEach
    void setUp() {
        // folder.getAccount().getId() 체인 대응
        folderMock = mock(WordbookFolder.class, RETURNS_DEEP_STUBS);
        when(folderMock.getAccount().getId()).thenReturn(accountId);

        // 공통: 폴더 존재/소유자 매칭
        when(wordbookFolderRepository.findById(folderId)).thenReturn(Optional.of(folderMock));

        // saveAll은 넘겨준 Iterable 그대로 반환
        when(wordbookTermRepository.saveAll(any(Iterable.class)))
                .thenAnswer(invocation -> {
                    Iterable<WordbookTerm> it = invocation.getArgument(0);
                    // Iterable 그대로 반환 (List로 감싸서 반환 형태 보존)
                    List<WordbookTerm> list = new ArrayList<>();
                    for (WordbookTerm u : it) list.add(u);
                    return list;
                });
    }
    ...
    void allNewTerms_shouldAttachAll_andSkipZero_andInvalidZero() {
        // given: 폴더엔 아무것도 없음
        when(wordbookTermRepository.findDistinctTermIdsByFolderAndAccountOrderByTermIdAsc(folderId, accountId))
                .thenReturn(List.of());
        when(wordbookTermRepository.findMaxSortOrderByAccountAndFolder(accountId, folderId))
                .thenReturn(0);
        ...
        verify(wordbookTermRepository, atLeastOnce())
    ...
    void wrongOwnerFolder_shouldThrowForbidden403() {
        ...
        WordbookFolder otherFolder = mock(WordbookFolder.class, RETURNS_DEEP_STUBS);
        when(otherFolder.getAccount().getId()).thenReturn(999L);
        when(wordbookFolderRepository.findById(folderId)).thenReturn(Optional.of(otherFolder));
